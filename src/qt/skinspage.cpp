...
SkinsPage::SkinsPage(QWidget *parent) : QWidget(parent), ui(new Ui::SkinsPage)
{
  ...
  QSettings settings("SpiderByte", "settings");
...
void SkinsPage::getlist()
{
  ...
  statusLabel->setText("<b>" + tr("Downloading themes from http://spiderbyte.co...") + "</b>");
  latestNetError = "";
  ...
  request.setUrl(QUrl("http://spiderbyte.co/themes/list.v2.txt"));
...
// calculate the md5 of remote and local files
	if (netHandleError(reply, "http://spiderbyte.co/themes/list.v2.txt"))
	{
...
void SkinsPage::getListFinished(QNetworkReply* reply)
{
	...
	// saves also the descriptor, will be used to check for an new version
		download((QString)"http://spiderbyte.co/themes/list.v2.txt");
		for (int i = 0; i < list.count(); i++)
		{
			...
			if (line.length())
			{
				...
				{  
					download("http://spiderbyte.co/themes/" + line);
				}
			}
		}
	}
