...
void Shutdown(void* parg)
{
	...
	// By Simone: changed the thread name to spiderbyte- instead of bitcoin-
    RenameThread("spiderbyte-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("SpiderByte version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  spiderbyted [options]                     " + "\n" +
                  "  spiderbyted [options] <command> [params]  " + _("Send command to -server or spiderbyted") + "\n" +
                  "  spiderbyted [options] help                " + _("List commands") + "\n" +
                  "  spiderbyted [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "SpiderByte:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("SpiderByte"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("SpiderByte"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: SpiderByte.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: SpiderByte.pid)") + "\n" +
...
//#ifndef WIN32
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  SpiderByte is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "SpiderByte server starting\n");
    ...
    if (boost::filesystem::exists(GetDataDir() / "wallet.dat"))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("SpiderByte"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of SpiderByte") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart SpiderByte to complete") << "\n";
