...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new SpiderByte address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current SpiderByte address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <SpiderByte-address> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid SpiderByte address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
		"sendtoaddress <SpiderByte-address> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <SpiderByte-address> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <SpiderByte-address> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <SpiderByte-address> [minconf=1]\n"
            "Returns the total amount received by <SpiderByte-address> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
		"sendfrom <fromaccount> <toSpiderByte-address> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a SpiderByte address or hex-encoded public key\n"
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("spiderbyte-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("spiderbyte-lock-wa");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; SpiderByte server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <SpiderByte-address>\n"
            "Return information about <SpiderByte-address>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <SpiderByte-pubkey>\n"
            "Return information about <SpiderByte-pubkey>.");
...
// SpiderByte: resend unconfirmed wallet transactions
...
Value zapwallettxes(const Array& params, bool fHelp)
{
  ...
  if (nLoadWalletRet != DB_LOAD_OK)
  {
    ...
    {
      printf("Error loading wallet.dat: Wallet requires newer version of SpiderByte\n");
      return("Error loading wallet.dat: Wallet requires newer version of SpiderByte\n");
    }
    else if (nLoadWalletRet == DB_NEED_REWRITE)
    {
      printf("Wallet needed to be rewritten: restart SpiderByte to complete\n");
      return("Wallet needed to be rewritten: restart SpiderByte to complete\n");
    }
    else
    {
